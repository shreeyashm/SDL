...
# load data and model
load('customer_churn.RData')
model_keras <- load_model_hdf5('customer_churn.hdf5', compile = FALSE)
...
output$customer_explanation <- renderBillboarder({
  ...
  billboarder() %>%
    ...
    bb_colors_manual('Less likely to churn' = 'rgba(1, 203, 148, 0.7)', 'More likely to churn' = 'rgba(118,6, 185, 0.7)')
})
```
...
bb_colors <- function(bb) {
  bb %>% bb_colors_manual('Yes' = 'rgba(1, 203, 148, 0.7)', 'No' = 'rgba(118, 6, 185, 0.7)')
}
```
  ...
  withProgress(message = 'Generating correlations plot', value = 0.6, {
    ...
    corrr_analysis %>%
      ...
      geom_point(color = palette_light()[[1]],
                 data = corrr_analysis %>% filter(Churn < 0)) +
      # Vertical lines
      geom_vline(xintercept = 0, color = palette_light()[[7]], size = 1, linetype = 2) +
      geom_vline(xintercept = -0.25, color = palette_light()[[7]], size = 1, linetype = 2) +
      geom_vline(xintercept = 0.25, color = palette_light()[[7]], size = 1, linetype = 2) +
      # Aesthetics
      theme_tq() +
